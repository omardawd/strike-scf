...
| Org-initiated KYB status transitions | Untraceable self-service status changes | **Still not audited** — `kyb_status` written directly with no log row in `kyb/status`, `onboarding/submit`, `onboarding/start` | Remaining Phase 1.J work | Eng | 🔴 |
| Role/permission changes | Untraceable privilege changes | `users.is_active` changes now write an `audit_events` row (`settings/team/[user_id]`). No route currently mutates an existing user's `role` field (confirmed via grep) — a non-issue until one exists, at which point it must be wired too | `lib/audit/log.ts` + migration `00000000000045`, tested | Eng | 🟢 (`is_active` only) |
| Bank account detail changes | Untraceable changes to disbursement destinations | Creation now writes an `audit_events` row (metadata + masked last-4 only, never full account/routing number). Edit/delete (`[id]/route.ts`) not yet wired | Same as above | Eng | 🟡 (create only) |
| Document access logging | Untraceable access to sensitive documents | `documents/[id]/url` now logs both successful signed-URL issuance and denied attempts | Same as above | Eng | 🟢 |
