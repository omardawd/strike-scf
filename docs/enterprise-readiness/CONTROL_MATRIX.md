...
| RLS as defense-in-depth | Cross-tenant access if service-role scoping is ever missed, or via Realtime/anon-key access | Enabled on all 45 tables; policies present on 34/45; **11 tables have RLS enabled with zero policies** (deny-all today, real gap once directly queried) | None automated; Phase 1.E adds pgTAP-style tests | Eng | 🟡 |
