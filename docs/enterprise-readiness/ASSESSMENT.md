...
**Fixed:** `supabase/migrations/00000000000044_dispatch_token_hardening.sql` adds `dispatch_token_hash`/`dispatch_token_prefix`/`dispatch_token_scopes`/`dispatch_token_expires_at`/`dispatch_token_revoked_at` columns, backfills the hash from every existing plaintext token (so already-configured integrations keep working — no forced reconnect), and drops the DB-level `DEFAULT`/`NOT NULL` on the old plaintext column so nothing populates it going forward. `lib/erp/dispatch-token.ts` generates the raw token in application code (never DB-generated), and `app/api/erp/connect/route.ts`'s connect/rotate response is now the *only* place the raw value is ever returned — `GET` (and `agents/activate`, which also used to leak it) return only `dispatch_token_prefix`. The Settings UI (`app/(portal)/settings/page.tsx`) now shows the full token exactly once, immediately after connect/rotate, with an explicit "save this now" warning, and a masked prefix thereafter. `app/api/ai/dispatch/route.ts` validates by hashing the presented token and comparing to `dispatch_token_hash` (never a plaintext match), rejects expired/revoked/inactive tokens, is now rate-limited (Track F, 20 req/min per org), supports `Idempotency-Key`-based replay protection (409 on reuse within 5 minutes), and restricts CORS to an explicit `DISPATCH_ALLOWED_ORIGINS` allowlist instead of `*` (empty by default — the endpoint's actual current callers are native apps and server-to-server webhooks, neither of which needs or sends CORS). **Known remaining gap:** the old plaintext `dispatch_token` column is intentionally not dropped yet (backward-compat for rows that haven't rotated since this shipped) — tracked in the migration's own comment and ROADMAP.md 1.G as a follow-up purge once confirmed safe. Per-tool scope enforcement (`dispatchTokenHasScope()`) is implemented but not yet wired into the tool-execution path — the column defaults every existing token to `'*'` (unrestricted), matching current behavior exactly; wiring per-tool scope checks is the next increment. Unit-tested (12 tests covering token generation, hash matching, expiry/revocation, and scope logic).
...
**Fixed (the filename-sanitization/path-safety part):** `lib/uploads/validate.ts` — `sanitizeFilename()` (strips to a safe character set, collapses `..` sequences even without a `/`, bounds length), `validateUpload()` (size cap + MIME allowlist), and a `scanForMalware()` extension point that always reports clean today (no scanner integrated — documented as Phase 2/3, same as before; wiring a real provider later needs no other code changes). Applied to all 3 previously-unsanitized routes (`onboarding/documents`, `transactions/[id]/documents`, `collateral/[id]`), each now also gets the default 20MB size cap. Unit-tested (12 tests). **Not yet done:** rolling the same helper out to the other 9 upload routes for consistent size/MIME enforcement (`ai/upload` already had its own equivalent size check; the rest are untouched) — tracked in ROADMAP.md 1.I as the remaining rollout, same pattern as the rate-limiter rollout in Track F.
...
- **P2-3 — Audit trail gaps for consequential actions.** ~~No audit row on: `users.role`/`is_active` changes~~, ~~`bank_accounts` creation/edit~~, ~~document access/signed-URL generation~~. **Partially fixed:** `supabase/migrations/00000000000045_audit_events.sql` adds a canonical, genuinely-append-only `audit_events` table — a DB-level `BEFORE UPDATE/DELETE` trigger raises on any mutation attempt, verified locally to reject even a superuser `UPDATE`/`DELETE`, not just an RLS-scoped one (RLS is bypassable by the service-role client the app uses everywhere; a trigger is not). `lib/audit/log.ts`'s `writeAuditEvent()` is the one write path, fails loud (logs + reports to error-tracking, never silently swallows) rather than fail-soft. Wired into 3 routes: `settings/team/[user_id]` (`is_active` changes — directly closes the P1-9 gap's audit side), `documents/[id]/url` (logs both successful signed-URL issuance and denied attempts), `settings/bank-accounts` (creation — logs only non-sensitive metadata + a masked last-4, never the full account/routing number). **Still open:** `settings/team/members` (new-member role assignment), org-initiated KYB status transitions (`kyb/status`, `onboarding/submit`, `onboarding/start`), `bank_accounts` edit/delete (`[id]/route.ts` — only creation is wired), and role changes generally (there is currently no route that changes an existing user's `role` field at all, per a repo-wide grep — only `is_active` is mutable today, which is what got wired). See ROADMAP.md 1.J for the explicit remaining list.
