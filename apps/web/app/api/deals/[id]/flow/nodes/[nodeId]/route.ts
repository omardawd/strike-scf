...
import { respondToFlowNode } from '@/lib/deals/flow'
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; nodeId: string }> }) {
  ...
  try {
    ...
    // Declining is always allowed; accepting commits to an optional
    // addition, so it requires admission like proposing does.
    if (body.response === 'accepted') {
      const admissionError = await assertOrgCanExpandDeal(adminClient, userData.org_id, 'accept a deal flow checkpoint')
      if (admissionError) return NextResponse.json({ error: admissionError }, { status: 403 })
    }
    const node = await respondToFlowNode({
      dealId: id,
      nodeId,
